...
import json
import MySQLdb
from MySQLdb._exceptions import OperationalError as OPE
...
class DBMain(object):
    ...
    def get_student_inform(self, account):
        sql = "SELECT stu.student_id,stu.student_name, en.unit_id, en.unit, en.title " \
              "FROM essential_english_words_1.unit as en, student.student_data as stu " \
              "WHERE stu.student_id = '{}' " \
              "and en.unit_id = (SELECT current_course FROM student.course_progress WHERE sid = '{}');"\
              .format(account, account)
        try:
            self.cursor.execute(sql)
        except (MySQLdb.OperationalError, OPE):
            self.cursor.execute(sql)
    ...
    def record_reading(self, sid, unit, content, speed, wer, confidence):
        ...
        try:
            self.cursor.execute(sql)
            self.db.commit()
        except (NameError, (MySQLdb.OperationalError, OPE)):
            self.operation_error()
            self.cursor.execute(sql)
            self.db.commit()
            return False
    ...
    def record_quiz_answer(self, student_reading_log):
        ...
        reading_log = student_reading_log
        sql_sentence = ''
